<?php
  ...
  $ext_valide = array('doc', 'docx', 'pdf', 'html');
  ...
  if ($b){
    ...
    if (!in_array($motiv_ext, $ext_valide)) echo $ext;
  }
  $an_id = $_POST['announce_id'];
  $user_id = $_POST['user_id'];
  $cv_name = "CV/".$an_id."CV".$user_id.".".$cv_ext;
  $res = move_uploaded_file($_FILES['CV']['tmp_name'], $cv_name);
  if ($b){
    $motiv_name = "Motiv/".$an_id."Motiv".$user_id.".".$motiv_ext;
    move_uploaded_file($_FILES['Motiv']['tmp_name'], $motiv_name);
    save_link_BD($connex, $b, $user_id, $an_id);
  }
?>
<!DOCTYPE html>
